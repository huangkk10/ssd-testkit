"""
Hibernate Performance Configured-Job Workflow Integration Test

Step-based end-to-end test for running Hibernate performance via the WAC
Configure Job path.  Survives the hibernate that WAC triggers as part of the
...
Workflow:
    ...
    Step 2 — Open WAC, configure Hibernate (Iterations=1), save custom job,
             connect launcher, persist reboot state, click Start.
             WAC will hibernate the machine immediately after Start.
    Step 3 — After hibernate resume: connect to WAC, wait for the View Results
             page, read errors / warnings / result path.
...
Requirements:
    ...
    - Hibernate must be enabled on the system.

Environment variables:
    ADK_HIBERNATE_JOB_NAME  Job name in the Save Custom Job dialog
                            (default: Hibernate_Workflow_Test)
    ADK_HIBERNATE_TIMEOUT   Max seconds to wait for the assessment result
                            (default: 7200 — 2 hours)
    ADK_LOG_DIR             Override the base directory for log output

Run:
    pytest tests/integration/lib/testtool/test_windows_adk/test_hibernate_workflow.py -v -s
"""
...
@pytest.mark.slow
class TestHibernateWorkflow(BaseTestCase):
    """
    Reboot-survivable end-to-end test for the Hibernate performance Configure Job workflow.
    ...
    # ------------------------------------------------------------------
    # Step 2 — Configure Hibernate and click Start (triggers hibernate)
    # ------------------------------------------------------------------

    @pytest.mark.order(2)
    @step(2, "Configure Hibernate and click Start")
    def test_02_configure_and_start(self, adk_env, check_environment):
        """
        Open WAC, configure Hibernate performance with Iterations=1, save the
        custom job, connect the Assessment Launcher.
        ...
        """
        job_name = os.getenv("ADK_HIBERNATE_JOB_NAME", "Hibernate_Workflow_Test")
        ctrl = ADKController(config={"log_path": self.log_path})

        ctrl._ui.open(WAC_EXE)
        ctrl._ui.select_hibernate_configured_job(num_iters=1)
    ...
    # ------------------------------------------------------------------
    # Step 3 — Wait for assessment result (runs after hibernate resume)
    # ------------------------------------------------------------------
    ...
    def test_03_wait_result(self, adk_env, check_environment):
        """
        Connect to WAC after the post-Hibernate resume and wait for the
        View Results page to appear.

        WAC auto-resumes the assessment on wakeup; when finished it switches
        to the View Results page.  We read:
        ...
        """
        timeout = int(os.getenv("ADK_HIBERNATE_TIMEOUT", "7200"))
        ctrl = ADKController(config={"log_path": self.log_path})

        # ── Verify this was a hibernate resume, not a cold boot ────────────
        # Hibernate assessment expects a hibernate resume (type=1).
        # A cold boot (type=2) would indicate something went wrong.
        # Event ID 1 data[0]=1  → Fast Startup / hibernate resume
        # Event ID 1 data[0]=2  → Cold boot (unexpected for Hibernate assessment)
        try:
            ...
            if boot_type == "1":
                logger.info("[TEST_03] Boot type confirmed: hibernate resume (expected for Hibernate assessment)")
            elif boot_type == "2":
                logger.warning(
                    "[TEST_03] Unexpected cold boot detected — "
                    "expected a hibernate resume.  Proceeding anyway."
                )
            else:
        ...
        TestHibernateWorkflow._wac_result = wac_result

        assert wac_result.errors == 0, (
            f"Hibernate assessment completed with {wac_result.errors} error(s). "
    ...
    def test_04_verify(self, adk_env, check_environment):
        ...
        wac_result = getattr(TestHibernateWorkflow, "_wac_result", None)
