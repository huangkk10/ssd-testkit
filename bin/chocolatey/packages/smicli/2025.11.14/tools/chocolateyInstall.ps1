...
$destExe = Join-Path $installDir "SmiCli2.exe"
if (-not (Test-Path $destExe)) {
    throw "Copy failed: SmiCli2.exe not found at $destExe"
}

# Set SMICLI_PATH machine-level env var so RunCard.generate_dut_info() finds it
# without needing an explicit path argument.
[Environment]::SetEnvironmentVariable('SMICLI_PATH', $destExe, 'Machine')
Write-Host "Set SMICLI_PATH = $destExe (Machine scope)"
