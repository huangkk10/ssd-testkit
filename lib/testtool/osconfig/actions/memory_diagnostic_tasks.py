"""
OsConfig — MemoryDiagnosticTasksAction

Disables the Windows Memory Diagnostic scheduled task.

Task path: ``\\Microsoft\\Windows\\MemoryDiagnostic\\RunFullMemoryDiagnostic``

Commands::

    apply()  → schtasks /Change /TN "\\Microsoft\\Windows\\MemoryDiagnostic\\RunFullMemoryDiagnostic" /DISABLE
    revert() → schtasks /Change /TN "\\Microsoft\\Windows\\MemoryDiagnostic\\RunFullMemoryDiagnostic" /ENABLE
    check()  → schtasks /Query /TN "\\Microsoft\\Windows\\MemoryDiagnostic\\RunFullMemoryDiagnostic" /FO LIST
...
_TASK_NAME = r"\Microsoft\Windows\MemoryDiagnostic\RunFullMemoryDiagnostic"
_CAP_KEY   = "memory_diagnostic_tasks"
...
class MemoryDiagnosticTasksAction(AbstractOsAction):
    """
    Disable the Windows Memory Diagnostic (RunFullMemoryDiagnostic) scheduled task.

    Args:
        snapshot_store: Optional shared snapshot dict.
    """

    name = "MemoryDiagnosticTasksAction"
    ...
    def check(self) -> bool:
        """Return ``True`` when the RunFullMemoryDiagnostic task is Disabled."""
        return _query_task_disabled(_TASK_NAME)

    def apply(self) -> None:
        """Disable the RunFullMemoryDiagnostic task."""
        ...
        logger.debug(f"[{self.name}] RunFullMemoryDiagnostic disabled")
        self._log_apply_done()

    def revert(self) -> None:
        """Re-enable the RunFullMemoryDiagnostic task."""
