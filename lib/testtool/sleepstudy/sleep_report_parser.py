"""
SleepStudy Sleep Report Parser
...
from .exceptions import SleepStudyLogParseError
...
class SleepReportParser:
    ...
    Raises:
        :class:`~.exceptions.SleepStudyLogParseError`: if the file does not
            exist, cannot be read, or does not contain the expected JSON payload.
    ...
    def __init__(self, html_path: str) -> None:
        ...
        if not self.html_path.exists():
            raise SleepStudyLogParseError(f"Sleep study HTML not found: {self.html_path}")
    ...
    def get_sleep_sessions(
        ...
        Raises:
            :class:`~.exceptions.SleepStudyLogParseError`: on parse failure.
        """
    ...
    def _load_data(self) -> dict:
        ...
        except SleepStudyLogParseError as regex_err:
            logger.debug(
    ...
    def _extract_json_via_playwright(self) -> dict:
        ...
        Raises:
            :class:`~.exceptions.SleepStudyLogParseError`: if Playwright fails or
    ...
    def _extract_json_via_regex(self) -> dict:
        ...
        Raises:
            :class:`~.exceptions.SleepStudyLogParseError`: if the pattern is
                not found or the JSON is malformed.
        """
    ...
    def _parse_dt_arg(dt_str: str) -> datetime:
        ...
        Raises:
            :class:`~.exceptions.SleepStudyLogParseError`: on invalid format.
        """
        for fmt in (
            "%Y-%m-%dT%H:%M:%S",
            "%Y-%m-%d %H:%M:%S",
